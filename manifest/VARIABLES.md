...
| ManifestShape | manifest | license | 1 |  | Defines the license(s) valid for all content referenced in the manifest. Does not apply to linked data(sets) with explicit license terms. |  | manifest_shacl.ttl |
| ManifestShape | manifest | content | 1 |  | Defines the links to all relevant data (sets) for a domain specific asset. This is extended and structures in the asset specific ontology. |  | manifest_shacl.ttl |
