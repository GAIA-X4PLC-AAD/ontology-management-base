...
def main():
    ...
    try:
        ...
        # Step 3: Process each ontology.ttl file and its corresponding shacl.ttl file
        ...
        if DRY_RUN:
            print("DRY RUN: All files processed successfully.")
        else:
...
def process_ontology_file(
    ...
    if ontology_graph_catalogue:
        ...
        add_shape(auth_header, ontology_graph_filesystem, directory_path)
...
def extract_ontology_name(graph: rdflib.Graph) -> Optional[str]:
    ...
    for row in results:
        return str(row.ontology)  # Return the ontology name
    return None
...
def add_shape(auth_header: dict, ontology_graph: rdflib.Graph, directory_path: str):
    """
