...
def normalize_text(text):
    ...
    text = re.sub(r'([^\w\s:/.-])', '', text)  # Preserve colons, slashes, periods, and dashes
    return text


def extract_validation_result(output):
    """Extracts the SHACL validation summary, ensuring 'Conforms: False' is captured."""
    match = re.search(r'Validation Report\s+Conforms:\s+(True|False)', output, re.IGNORECASE)
...
def run_command(command, description):
    ...
    try:
        ...
        print("\n🔎 Extracted & Normalized Output:\n" + normalized_output)  # 🔍 Debugging step
...
def ensure_shacl_failure(test_name, actual_output):
    ...
    if "conforms: false" not in actual_output:
        print(f"❌ Test {test_name} should fail, but SHACL validation reported success!")
...
def check_ttl_syntax():
    ...
    for ontology in ONTOLOGY_DIRS:
        ttl_files = [os.path.join(ontology, f) for f in os.listdir(ontology) if f.endswith(".ttl")]
...
def check_failing_tests():
    ...
    for ontology in ONTOLOGY_DIRS:
        ...
        fail_tests = [f for f in os.listdir(test_folder) if f.startswith("fail") and f.endswith(".json")]
        ...
        for test in instance_files:
            ...
            returncode, actual_output = run_command(
                [sys.executable, os.path.join(SRC_DIR, "check_jsonld_against_shacl_schema.py")] + validation_args,
            ...
            if os.path.exists(expected_output_path):
                ...
                if expected_output in actual_output:
                    ...
                    diff_lines = [f"- {line}" for line in expected_output.split("\n") if line not in actual_output.split("\n")]
                    extra_lines = [f"+ {line}" for line in actual_output.split("\n") if line not in expected_output.split("\n")]
    
                    print("\n🔎 Differences Detected:\n" + "\n".join(diff_lines + extra_lines))
                    sys.exit(1)
            else:
                print(f"⚠️ No expected output file found for {test}. Creating placeholder.")
                with open(expected_output_path, "w") as placeholder:
                    placeholder.write(actual_output)
                print(f"📄 Created {expected_output_path} with actual validation output.")
