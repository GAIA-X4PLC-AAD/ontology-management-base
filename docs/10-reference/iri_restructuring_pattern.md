...
4. **Alignment with Repository Registry**: The validation tool (`conformance_validator.py` / `validation_suite`) uses a Repository Registry that indexes files by their IRI. By declaring the shapes graph as an ontology with a formal IRI, it becomes discoverable and resolvable through the registry's semantic dependency resolution mechanism.
...
```bash
python3 -m src.tools.validators.validation_suite --run check-data-conformance --path scenario/scenario_instance.json
```
