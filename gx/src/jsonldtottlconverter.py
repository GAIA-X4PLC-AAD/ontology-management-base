...
SOURCE_FOLDER = os.path.join(
    os.getcwd(), "../input/01_original"
)  # folder, where jsonld files are stored in .json format
CORRECTION_FOLDER = os.path.join(
    os.getcwd(), "../input/02_corrected"
)  # folder, where corrected jsonld files are stored in .json format
TARGET_FOLDER = os.path.join(
    os.getcwd(), "../"
)  # folder, where the resulting .ttl files shall be written to
...
def correct_shape_suffix(filepath):
    ...
    filedata = filedata.replace(
        '"@id": "gx:DataResource",', '"@id": "gx:DataResourceShape",', 1
    )
    filedata = filedata.replace(
        '"@id": "gx:InstantiatedVirtualResource",',
        '"@id": "gx:InstantiatedVirtualResourceShape",',
        1,
    )
    filedata = filedata.replace(
        '"@id": "gx:LegitimateInterest",', '"@id": "gx:LegitimateInterestShape",', 1
    )
    filedata = filedata.replace(
        '"@id": "gx:ServiceAccessPoint",', '"@id": "gx:ServiceAccessPointShape",', 1
    )
    filedata = filedata.replace(
        '"@id": "gx:SoftwareResource",', '"@id": "gx:SoftwareResourceShape",', 1
    )
    filedata = filedata.replace(
        '"@id": "gx:VirtualResource",', '"@id": "gx:VirtualResourceShape",', 1
    )
    filedata = filedata.replace(
        '"@id": "gx:PhysicalResource",', '"@id": "gx:PhysicalResourceShape",', 1
    )
    # Write the file out again
    with open(filepath, "w") as file:
        file.write(filedata)
...
def correct_legal_participant(filepath):
    ...
    with open(filepath, "w") as file:
        file.write(filedata)
    print(
        f"  LegalPerson to LegalParticipant in file {filepath} were successfully corrected."
    )
...
def correct_sub_class_of_attribute(filepath):
    ...
    filedata = filedata.replace(
        ...
        '        "@id": "https://w3id.org/gaia-x/core#Participant"',
    )
    filedata = filedata.replace(
        '        "@id": "http://w3id.org/gaia-x/core#Resource"',
        '        "@id": "https://w3id.org/gaia-x/core#Resource"',
    )
    filedata = filedata.replace(
        '        "@id": "http://w3id.org/gaia-x/core#ServiceOffering"',
        '        "@id": "https://w3id.org/gaia-x/core#ServiceOffering"',
    )
...
def correct_legal_registration_number(filepath):
    ...
    print(
        f"  Legal registration number section in file {filepath} was successfully corrected."
    )
...
def correct_missing_legal_registration_number(filepath):
    ...
    new_element = {
        ...
        "http://www.w3.org/2000/01/rdf-schema#label": [
            {"@language": "en", "@value": "Legal Registration Number"}
        ],
    }
...
def correct_link_from_class_to_node(filepath):
    ...
    filedata = re.sub(
        pattern_service_offering,
        replacement_service_offering,
        filedata,
        flags=re.MULTILINE,
    )
    ...
    replacement_legal_participant = (
        '"sh:node": {\n            "@id": "gx:LegalParticipant"\n          },'
    )
    filedata = re.sub(
        pattern_legal_participant,
        replacement_legal_participant,
        filedata,
        flags=re.MULTILINE,
    )
...
process_file(
    ...
    [
        correct_namespace,
        correct_legal_participant,
        correct_sub_class_of_attribute,
...
process_file(
    SHACL_JSON_FILE,
    [
        correct_shape_suffix,
        correct_address_shape,
        correct_legal_registration_number,
